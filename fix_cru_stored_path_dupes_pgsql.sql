...
INSERT INTO tmp_cru_stored_path_conversion ( old_id, new_id )
SELECT sp.cru_path_id old_id,
       tmp.min_id new_id
FROM
  (SELECT Min(cru_path_id) min_id,
           cru_path
   FROM cru_stored_path
   GROUP BY cru_path) tmp
JOIN cru_stored_path sp ON tmp.cru_path = sp.cru_path;

CREATE TABLE tmp_cru_revision_conversion ( new_id INT, old_id INT );

INSERT INTO tmp_cru_revision_conversion ( new_id, old_id )
  (SELECT tmp.min_id,
           rev.cru_revision_id
   FROM cru_revision rev
   JOIN tmp_cru_stored_path_conversion spconv ON rev.cru_path = spconv.old_id
   JOIN
     (SELECT Min(rev.cru_revision_id) min_id,
              rev.cru_source_name,
              spconv.new_id,
              rev.cru_revision
      FROM cru_revision rev
      ...
      GROUP BY rev.cru_source_name,
               spconv.new_id,
               rev.cru_revision) tmp ON tmp.cru_source_name = rev.cru_source_name
   AND tmp.new_id = spconv.new_id
   AND tmp.cru_revision = rev.cru_revision );

 -- update cru_revision dependencies

UPDATE cru_feindex_msg
SET cru_fr_id = conv.new_id
FROM cru_feindex_msg fm
JOIN tmp_cru_revision_conversion conv ON fm.cru_fr_id = conv.old_id;

UPDATE cru_patch_revision
SET cru_revision_id = conv.new_id
FROM cru_patch_revision pr
JOIN tmp_cru_revision_conversion conv ON pr.cru_revision_id = conv.old_id;
...
UPDATE cru_frx_revision
SET cru_revision = conv.new_id
FROM cru_frx_revision fr
JOIN tmp_cru_revision_conversion conv ON fr.cru_revision = conv.old_id;

 -- remove duplicates from cru_revisions

ALTER TABLE cru_revision DROP CONSTRAINT uk_crurevision_spr;

UPDATE cru_revision rev
SET cru_path = conv.new_id
FROM tmp_cru_stored_path_conversion conv
WHERE rev.cru_path = conv.old_id;
...
ALTER TABLE cru_revision ADD CONSTRAINT uk_crurevision_spr UNIQUE (cru_source_name,
                                                                   cru_path,
                                                                   cru_revision);
...
UPDATE cru_stored_path SET cru_hash = Cast(Encode(Digest(cru_path,'sha256'),'hex') AS VARCHAR(128));
